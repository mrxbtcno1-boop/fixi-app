...
import {
  ...
  FlatList, ScrollView, KeyboardAvoidingView, Platform, Animated, Image,
} from 'react-native';
...
export default function AiCoachScreen() {
  ...
  const selectedEmotions = useAppStore(s => s.selectedEmotions);
  ...
  const addWelcomeMessage = () => {
    setMessages([{
      id: 'welcome',
      role: 'assistant',
      content: `Hey ${userName || 'du'}! 🦊 Lass mich direkt sein: Mit dem richtigen Plan wirst du früher schuldenfrei als du denkst. Womit soll ich dir heute helfen?`,
    }]);
  };
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      // Prepend emotional context from onboarding for personalized coaching
      const emotionsPrefix: { role: 'system'; content: string }[] = selectedEmotions.length > 0
        ? [{ role: 'system', content: `Der Nutzer fühlt sich aktuell: ${selectedEmotions.join(', ')}.` }]
        : [];
      const historyWithEmotions = [...emotionsPrefix, ...chatHistory.slice(-10)];
      const response = await sendToCoach(text.trim(), historyWithEmotions, userData);
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 17, fontWeight: '600', color: Colors.text.primary, fontFamily: 'Nunito_900Black' },
  headerSub: { fontSize: 12, color: Colors.text.tertiary, marginTop: 2 },
  headerFoxWrap: { width: 44, height: 44, backgroundColor: 'transparent' },
  headerFoxImg: { width: 44, height: 44, backgroundColor: 'transparent' },
  chatList: { flex: 1 },
  ...
  fixiAvatar: { width: 36, height: 36, backgroundColor: 'transparent', alignItems: 'center', justifyContent: 'center', marginRight: 6 },
  fixiAvatarImg: { width: 36, height: 36, backgroundColor: 'transparent' },
